...
public class Scanner {
	...
	void CheckLiteral() {
		switch (t.val.ToLower()) {
			...
			case "not": t.kind = 7; break;
			case "like": t.kind = 8; break;
			case "escape": t.kind = 9; break;
			case "between": t.kind = 10; break;
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 1:
				...
				if (ch == '.' || ch >= '0' && ch <= '9' || ch == '_' || ch >= 'a' && ch <= 'z' || ch == 128 || ch >= 160 && ch <= 179 || ch == 181 || ch == 186 || ch >= 192 && ch <= 214 || ch >= 216 && ch <= 246 || ch >= 248 && ch <= 255) {AddCh(); goto case 1;}
