...
namespace QueryTests
{
	...
	public class NDOLinqTests
	{
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( pm );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter]" == q.GeneratedQuery );
		}
		...
		public void LinqCheckMitarbeiterQuery()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter]" == qs );
		}
		...
		public void CheckIfOrderingsWork()
		{
			...
			string qs = vt.QueryString;
			var should = $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] ORDER BY [Mitarbeiter].[Vorname] ASC, [Mitarbeiter].[Nachname] DESC";
            Assert.That( qs, Is.EqualTo(should) );

			vt = pm.Objects<Mitarbeiter>();
			vt.OrderBy( m => m.Vorname ).OrderByDescending( m => m.Nachname );
			vt.Take( 10 ).Skip( 12 );
			qs = vt.QueryString;
			should = $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] ORDER BY [Mitarbeiter].[Vorname] ASC, [Mitarbeiter].[Nachname] DESC OFFSET 12 ROWS FETCH NEXT 10 ROWS ONLY";
            Assert.That( qs, Is.EqualTo(should) );
		}
		...
		public void LinqTestIfSimpleWhereClauseWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] = {{0}}" == vt.QueryString );
			// Query for Oid values
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Oid() == 5 );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[ID] = {{0}}" == vt.QueryString );
		...
		public void LinqCheckIfWhereClauseWith1nRelationWorks()
		{
			...
			string qs = vt.QueryString;
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[Zweck] = {{0}}" == qs );
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Reisen[Any.Index].Oid().Equals( 5 ) );
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[ID] = {{0}}" == vt.QueryString );
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[ID] = {{0}}", Is.EqualTo( vt.QueryString ) );
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Reisen[Any.Index].NDOObjectId.In(new int[] { 1, 2, 3 } ) );
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[ID] IN (1, 2, 3)" == vt.QueryString );
		}
		...
		public void LinqCheckIfWhereClauseWith11RelationWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Adresse] ON [Adresse].[ID] = [Mitarbeiter].[IDAdresse] WHERE [Adresse].[Lkz] LIKE {{0}}" == vt.QueryString );
		}
		...
		public void LinqCheckIfWhereClauseWithOidIn11RelationWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[IDAdresse] = {{0}}" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Adresse.Oid().In(new[]{ 1, 2, 3 } ) );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[IDAdresse] IN (1, 2, 3)" == vt.QueryString );
		}
		...
		public void LinqCheckOidWithTable()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] INNER JOIN [relLandReise] ON [Reise].[ID] = [relLandReise].[IDReise] WHERE [relLandReise].[IDLand] = {{0}}" == vt.QueryString );
		}
		...
		public void LinqCheckThatOneJoinAppearsOnlyOneTime()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Adresse] ON [Adresse].[ID] = [Mitarbeiter].[IDAdresse] WHERE [Adresse].[Lkz] LIKE {{0}} AND [Adresse].[Ort] <> {{1}}" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Reisen[Any.Index].Zweck == "ADC" || m.Reisen[Any.Index].Zweck == "ADW" );
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[Zweck] = {{0}} OR [Reise].[Zweck] = {{1}}" == vt.QueryString );
		}
		...
		public void LinqCheckNotOperator()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE NOT ([Mitarbeiter].[Nachname] = {{0}})" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Vorname.Like( "M%" ) && !(m.Nachname == "Matytschak") );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] LIKE {{0}} AND NOT ([Mitarbeiter].[Nachname] = {{1}})" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => !(m.Vorname.Like( "M%" ) && m.Nachname == "Matytschak") );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE NOT ([Mitarbeiter].[Vorname] LIKE {{0}} AND [Mitarbeiter].[Nachname] = {{1}})" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Reisen[Any.Index].Zweck == "ADC" || !(m.Reisen[Any.Index].Zweck == "ADW") );
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[Zweck] = {{0}} OR NOT ([Reise].[Zweck] = {{1}})" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => !(m.Reisen[Any.Index].Zweck == "ADC" || m.Reisen[Any.Index].Zweck == "ADW") );
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE NOT ([Reise].[Zweck] = {{0}} OR [Reise].[Zweck] = {{1}})" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => !(m.Reisen[Any.Index].Länder[Any.Index].IsInEu == true) );
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] INNER JOIN [relLandReise] ON [Reise].[ID] = [relLandReise].[IDReise] INNER JOIN [Land] ON [Land].[ID] = [relLandReise].[IDLand] WHERE NOT ([Land].[IsInEu] = {{0}})" == vt.QueryString );
		}
		...
		public void LinqCheckBetween()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] BETWEEN {{0}} AND {{1}}" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => !m.Vorname.Between( "A", "B" ) );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE NOT [Mitarbeiter].[Vorname] BETWEEN {{0}} AND {{1}}" == vt.QueryString );
		}
		...
		public void LinqTestValueType()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Position_X] > {{0}} AND [Mitarbeiter].[Position_Y] < {{1}}" == vt.QueryString );
		}
		...
		public void TestValueTypeRelation()
		{
			...
			Assert.That( $"SELECT {fields} FROM [Sozialversicherungsnummer] INNER JOIN [Mitarbeiter] ON [Mitarbeiter].[ID] = [Sozialversicherungsnummer].[IDSozial] WHERE [Mitarbeiter].[Position_X] > {{0}} AND [Mitarbeiter].[Position_Y] < {{1}}" == vt.QueryString );
		}
		...
		public void LinqCheckIfMultiKeysWork()
		{
			...
			Assert.That( $"SELECT {fields} FROM [OrderDetail] WHERE [OrderDetail].[IDProduct] = {{0}} AND [OrderDetail].[IDOrder] = {{1}}" == vt.QueryString );
#if ignored
			bool thrown = false;
			try
			{
		...
		public void LinqTestBooleanExpression()
		{
			...
			Assert.That( $"SELECT {fields} FROM [Land] WHERE [Land].[IsInEu] = {{0}}" == vt.QueryString );

			vt = pm.Objects<Land>().Where( l => l.IsInEu );
			Assert.That( $"SELECT {fields} FROM [Land] WHERE [Land].[IsInEu] = 1" == vt.QueryString );

			vt = pm.Objects<Land>().Where( l => l.IsInEu && l.Name == "Lala" );
			Assert.That( $"SELECT {fields} FROM [Land] WHERE [Land].[IsInEu] = 1 AND [Land].[Name] = {{0}}" == vt.QueryString );
		...
		public void CanAddPrefetches()
		{
			...
			Assert.That( 2 == list.Count );
			Assert.That( "Reisen" == list[0] );
			Assert.That( "Reisen.Länder" == list[1] );
		}
		...
		public void LinqTest1To1()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Buero] ON [Buero].[ID] = [Mitarbeiter].[IDBuero] WHERE [Buero].[Zimmer] = {{0}}" == vt.QueryString );
		}
		...
		public void LinqTest1To1Bidirectional()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Sozialversicherungsnummer] ON [Sozialversicherungsnummer].[ID] = [Mitarbeiter].[IDSozial] WHERE [Sozialversicherungsnummer].[Nummer] = 'abc'" ==  q.GeneratedQuery );
            var vt2 = pm.Objects<Sozialversicherungsnummer>().Where(s=>s.Angestellter.Vorname == "Mirko");
			var fields = new SqlColumnListGenerator( pm.NDOMapping.FindClass( typeof( Sozialversicherungsnummer ) ) ).Result( false, false, true );
			Assert.That( $"SELECT {fields} FROM [Sozialversicherungsnummer] INNER JOIN [Mitarbeiter] ON [Mitarbeiter].[ID] = [Sozialversicherungsnummer].[IDSozial] WHERE [Mitarbeiter].[Vorname] = {{0}}" == vt2.QueryString );
		}
		...
		public void LinqTest1To1BiWithTable()
		{
			...
			Assert.That( $"SELECT {fields} FROM [Zertifikat] INNER JOIN [relSignaturZertifikat] ON [Zertifikat].[ID] = [relSignaturZertifikat].[IDZertifikat] INNER JOIN [Signatur] ON [Signatur].[ID] = [relSignaturZertifikat].[IDSignatur] WHERE [Signatur].[Signature] = {{0}}" == vt1.QueryString );
			...
			Assert.That( $"SELECT {fields} FROM [Signatur] INNER JOIN [relSignaturZertifikat] ON [Signatur].[ID] = [relSignaturZertifikat].[IDSignatur] INNER JOIN [Zertifikat] ON [Zertifikat].[ID] = [relSignaturZertifikat].[IDZertifikat] WHERE [Zertifikat].[Schlüssel] = {{0}}" == vt2.QueryString);
		}
		...
		public void LinqTest1ToNWithTable()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [relMitarbeiterReisebuero] ON [Mitarbeiter].[ID] = [relMitarbeiterReisebuero].[IDMitarbeiter] INNER JOIN [Reisebuero] ON [Reisebuero].[ID] = [relMitarbeiterReisebuero].[IDReisebuero] WHERE [Reisebuero].[Name] = {{0}}" == vt.QueryString );
		}
		...
		public void LinqTestIfQueryForNonNullOidsWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] INNER JOIN [relLandReise] ON [Reise].[ID] = [relLandReise].[IDReise] WHERE [relLandReise].[IDLand] IS NOT NULL" == vt.QueryString );
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[IDAdresse] IS NOT NULL" == vt.QueryString );
		}
		...
		{
			var vt = pm.Objects<Mitarbeiter>().Where( m => m.Adresse == null );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[IDAdresse] IS NULL" == vt.QueryString );
		}
		...
		public void LinqTestIfIsNullWithStringWorks()
		{
			...
			Assert.That( sql == vt.QueryString );
		...
		public void LinqTestIfIsNotNullWithStringWorks()
		{
			...
			Assert.That( sql == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => !m.Vorname.Equals( null ) );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE NOT [Mitarbeiter].[Vorname] IS NULL" == vt.QueryString );
		}
		...
		public void LinqTestIfInClauseWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] IN ('Mirko', 'Hans')", Is.EqualTo( vt.QueryString ) );
            vt = pm.Objects<Mitarbeiter>().Where(m => m.Vorname.In(new[] { "Mirko", "Hans" }));
            Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] IN ('Mirko', 'Hans')", Is.EqualTo( vt.QueryString) );
			arr = new[] { "Mirko" };
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Vorname.In( arr ) );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] IN ('Mirko')", Is.EqualTo( vt.QueryString ) );
		}
		...
		public void LinqTestIfInClauseWithNumbersWorks()
		{
            ...
            var vt = pm.Objects<Mitarbeiter>().Where(m => m.Vorname.In(arr));
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] IN (1, 2, 3, 4, 5)" == vt.QueryString );
			vt = pm.Objects<Mitarbeiter>().Where( m => !m.Oid().In( arr ) );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE NOT [Mitarbeiter].[ID] IN (1, 2, 3, 4, 5)" == vt.QueryString );
			arr = new[] { 333 };
			vt = pm.Objects<Mitarbeiter>().Where( m => !m.Oid().In( arr ) );
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE NOT [Mitarbeiter].[ID] IN (333)" == vt.QueryString );
		}
		...
		public void LinqTestIfInClauseWithGuidsWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] IN ('{guids[0]}', '{guids[1]}')" == vt.QueryString );
		}
		...
		public void LinqTestIfRelationInInClauseWorks()
		{
            ...
            Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[ID] IN (1, 2, 3, 4, 5)" == vt.QueryString );
		...
		public void TestIfOidWithInClauseWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[ID] IN (1, 2, 3, 4, 5)" == vt.QueryString );
		}
		...
		public void TestIfLinqQueryForNonNullOidsWorks()
		{
			...
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] WHERE [Reise].[ID] IS NOT NULL" == qs );
			vt = pm.Objects<Mitarbeiter>().Where( m => m.Reisen[Any.Index].Länder[Any.Index].Oid() != null );
			qs = vt.QueryString;
			Assert.That( $"SELECT {this.mitarbeiterJoinFields} FROM [Mitarbeiter] INNER JOIN [Reise] ON [Mitarbeiter].[ID] = [Reise].[IDMitarbeiter] INNER JOIN [relLandReise] ON [Reise].[ID] = [relLandReise].[IDReise] WHERE [relLandReise].[IDLand] IS NOT NULL" == qs );
		}
		...
		public void CanFetchForNotStringIsNullOrEmpty()
		{
			...
			var qs = vt.QueryString;
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] IS NOT NULL AND [Mitarbeiter].[Vorname] <> {{0}}" == vt.QueryString );
		}
		...
		public void CanUseStringsInINClauses()
		{
			...
			var qs = vt.QueryString;
			Assert.That( $"SELECT {this.mitarbeiterFields} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] LIKE {{0}} AND NOT [Mitarbeiter].[Nachname] IN ('1', '2', '3')" == vt.QueryString );
		}
		...
		public void CanCallServerFunctions()
		{
			...
			var qs = vt.QueryString;
			Assert.That( qs == $"SELECT {this.reiseFields} FROM [Reise] WHERE [Reise].[Zweck] > TestFunction([Reise].[Zweck], {{0}})" );
			vt = pm.Objects<Reise>().Where( m => ServerFunctions.ParameterlessFunction() > 2143 );
			qs = vt.QueryString;
			Assert.That( qs == $"SELECT {this.reiseFields} FROM [Reise] WHERE ParameterlessFunction() > {{0}}" );
			...
			qs = vt.QueryString;
			Assert.That( qs == $"SELECT {this.reiseFields} FROM [Reise] WHERE JSON_VALUE([Reise].[Zweck], {{0}}) > {{1}}" );
			...
			qs = vt.QueryString;
			Assert.That( qs == $"SELECT {this.reiseFields} FROM [Reise] WHERE JSON_VALUE([Reise].[Zweck], {{0}}) LIKE {{1}}" );
		}
		...
		public void BoolOperatorsWork()
		{
			...
			Assert.That( "WHERE [Buero].[HatSchnellesInternet] = 1 OR [Buero].[HatCat6Anschluss] = 1" == qs.Substring( p ) );

			vt = pm.Objects<Buero>().Where( m => (m.HatSchnellesInternet || m.HatCat6Anschluss) == true );
			qs = vt.QueryString;
			p = qs.IndexOf( "WHERE" );
			Assert.That( "WHERE ([Buero].[HatSchnellesInternet] = 1 OR [Buero].[HatCat6Anschluss] = 1) = {0}" == qs.Substring( p ) );
			...
			qs = vt.QueryString;
			p = qs.IndexOf( "WHERE" );
			Assert.That( "WHERE [Buero].[HatSchnellesInternet] = 1 AND [Buero].[HatCat6Anschluss] = 1" == qs.Substring( p ) );

			vt = pm.Objects<Buero>().Where( m => (m.HatSchnellesInternet && m.HatCat6Anschluss) == true );
			qs = vt.QueryString;
			p = qs.IndexOf( "WHERE" );
			Assert.That( "WHERE ([Buero].[HatSchnellesInternet] = 1 AND [Buero].[HatCat6Anschluss] = 1) = {0}" == qs.Substring( p ) );
