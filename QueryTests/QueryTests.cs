...
namespace QueryTests
{
	...
	public class NDOQueryTests
	{
		...
		public void SetUp()
		{
			this._pm = NDOFactory.Instance.PersistenceManager;

			mitarbeiterFields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Mitarbeiter ) ) ).SelectList;
			mitarbeiterJoinFields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Mitarbeiter ) ) ).Result( false, false, true );
			belegFields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Beleg ) ) ).SelectList;
			this.pkwFahrtFields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( PKWFahrt ) ) ).SelectList;
			this.reiseFields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Reise ) ) ).SelectList;
			this.reiseJoinFields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Reise ) ) ).Result( false, false, true );
		...
		public void QueryWithEmptyGuidParameterSearchesForNull()
		{
			...
			var q = new NDOQuery<DataContainer>(_pm, "guidVar = {0}");
			q.Parameters.Add( Guid.Empty );
			var fields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( DataContainer ) ) ).SelectList;
			var sql = $"SELECT {fields} FROM [DataContainer] WHERE [DataContainer].[GuidVar] IS NULL";
			Assert.That( q.GeneratedQuery.StartsWith( sql ) );

			q = new NDOQuery<DataContainer>(_pm, "guidVar <> {0}");
			q.Parameters.Add( Guid.Empty );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname = 'Mirko'" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname LIKE 'M*'" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname = SqlFunction('Mirko')" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname = SqlFunction('Mirko', 42)" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname = SqlFunction()" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname = SqlFunction(nachname)" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "SqlFunction('Mirko', 42) > 3124" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname = {0}" );
		...
		public void OrderingByOidWorks()
		{
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname = {0}" );
			q.Parameters.Add( "Mirko" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "dieReisen.zweck = 'ADC'" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "dieReisen.oid = {0}" );
			q.Parameters.Add( 1 );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "adresse.lkz LIKE 'D%'" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "adresse.lkz LIKE 'D%' AND dieReisen.dieLaender.name = 'D'" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "dieReisen.dieLaender.oid = {0}" );
			q.Parameters.Add( 1 );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "adresse.lkz LIKE 'D%' AND adresse.ort <> 'Bad Tölz'" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "dieReisen.zweck = 'ADC' OR dieReisen.zweck = 'ADW'" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "NOT (vorname LIKE 'M%')" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname LIKE 'M%' AND NOT nachname = 'Matytschak'" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "NOT (vorname LIKE 'M%' AND nachname = 'Matytschak')" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "dieReisen.zweck = 'ADC' OR NOT dieReisen.zweck = 'ADW'" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "NOT (dieReisen.zweck = 'ADC' OR dieReisen.zweck = 'ADW')" );
			...
			try
			{
				q = new NDOQuery<Mitarbeiter>( _pm, "vorname LIKE 'M%' AND nachname = NOT 'Matytschak'" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname LIKE 'M%' AND nachname = NOT True" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname BETWEEN 'A' AND 'B'" );
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "NOT vorname BETWEEN 'A' AND 'B'" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "position.X > 2 AND position.Y < 5" );
		...
		{
			NDOQuery<Sozialversicherungsnummer> q = new NDOQuery<Sozialversicherungsnummer>( _pm, "arbeiter.position.X > 2 AND arbeiter.position.Y < 5" );
			var fields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Sozialversicherungsnummer ) ) ).Result( false, false, true );
		...
		{
			bool qeThrown = false;
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname IN 'Mirko'" );
			try
			{
		...
		{
			NDOQuery<OrderDetail> q = new NDOQuery<OrderDetail>( _pm, "oid = {0}" );
			var od = _pm.FindObject(typeof(OrderDetail), new object[]{ 1, 2 } );
			q.Parameters.Add( od.NDOObjectId );
			var fields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( OrderDetail ) ) ).SelectList;
			Assert.AreEqual( $"SELECT {fields} FROM [OrderDetail] WHERE [OrderDetail].[IDProduct] = {{0}} AND [OrderDetail].[IDOrder] = {{1}}", q.GeneratedQuery );
			bool thrown = false;
			try
			{
				q = new NDOQuery<OrderDetail>( _pm, "oid = -4" );
				string s = q.GeneratedQuery;
			}
			catch (NDOException)
			{
		...
		public void CheckIfSqlQueryIsProcessed()
		{
			...
			var handler = handlerMock.Object;
			var container = pm.ConfigContainer;
			container.RegisterInstance<IPersistenceHandlerManager>( new TestPersistenceHandlerManager( handler ) );
		...
		public void CheckIfSingleKeyNDOObjectIdParameterIsProcessed()
		{
			...
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( pm, "oid = {0}" );
			var dummy = pm.FindObject( typeof( Mitarbeiter ), 121 );
		...
		public void SimpleQueryWithHandler()
		{
			...
			handlerMock.Setup( h => h.PerformQuery( It.IsAny<string>(), It.IsAny<IList>(), It.IsAny<DataSet>() ) ).Returns( new DataTable() ).Callback<string, IList, DataSet>( ( s, l, d ) => generatedParameters = l );
			var container = pm.ConfigContainer;
			container.RegisterInstance<IPersistenceHandlerManager>( new TestPersistenceHandlerManager( handlerMock.Object ) );
		...
		{
			NDOQuery<Kostenpunkt> qk = new NDOQuery<Kostenpunkt>( _pm );
		...
		{
			NDOQuery<Reise> q = new NDOQuery<Reise>( _pm, "belege.datum = {0}" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "meinBuero.zimmer = 'abc'" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "sn.nummer = 4711" );
			...
			NDOQuery<Sozialversicherungsnummer> qs = new NDOQuery<Sozialversicherungsnummer>( _pm, "arbeiter.vorname = 'Mirko'" );
			var fields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Sozialversicherungsnummer ) ) ).Result( false, false, true );
		...
		{
			NDOQuery<Zertifikat> qz = new NDOQuery<Zertifikat>( _pm, "sgn.signature = 'abc'" );
			var fields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Zertifikat ) ) ).Result( false, false, true );
			...
			NDOQuery<Signatur> qs = new NDOQuery<Signatur>( _pm, "owner.schlüssel = -4" );
			string s = qs.GeneratedQuery;
			fields = new SqlColumnListGenerator( _pm.NDOMapping.FindClass( typeof( Signatur ) ) ).Result( false, false, true );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "reiseBüros.name = 'abc'" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "dieReisen.dieLaender.oid IS NOT NULL" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "adresse IS NOT NULL" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "adresse IS NULL" );
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname IN (1,2,3,4,5)" );
			var s = q.GeneratedQuery;
		...
		public void TestIfSingleQuotesAreForbidden()
		{
			...
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname = 'lala'--SELECT * FROM Mitarbeiter'" );
			var s = q.GeneratedQuery;
			...
			q = new NDOQuery<Mitarbeiter>( _pm, "vorname = 'lala' -SELECT * FROM Mitarbeiter'" );
			try
			{
				s = q.GeneratedQuery;
			}
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "vorname IN ('1''','2''3','''3','4','5')" );
			var s = q.GeneratedQuery;
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "dieReisen.oid IN (1,2,3,4,5)" );
			var s = q.GeneratedQuery;
		...
		{
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( _pm, "oid IN (1,2,3,4,5)" );
			var s = q.GeneratedQuery;
		...
		public void TestIfSqlCodeWorks()
		{
			...
			NDOQuery<Mitarbeiter> q = new NDOQuery<Mitarbeiter>( this._pm, expression, false, QueryLanguage.Sql );
