...
namespace QueryTests
{
    ...
    public class NDOQueryTests
    {
		...
		string mitarbeiterFields = "[Mitarbeiter].[Nachname], [Mitarbeiter].[Position_X], [Mitarbeiter].[Position_Y], [Mitarbeiter].[Vorname]";
		string belegFields = "[Beleg].[Belegart], [Beleg].[Betrag], [Beleg].[Datum]";
		string pkwFahrtFields = "[PKWFahrt].[Datum], [PKWFahrt].[Km]";
		...
		public void CheckBetween()
		{
			...
			Assert.AreEqual( String.Format("SELECT {0} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Vorname] BETWEEN 'A' AND 'B'", this.mitarbeiterFields), q.GeneratedQuery );
		}
		...
		public void TestValueType()
		{
			...
			Assert.AreEqual( String.Format("SELECT {0} FROM [Mitarbeiter] WHERE [Mitarbeiter].[Position_X] > 2 AND [Mitarbeiter].[Position_Y] < 5", this.mitarbeiterFields), q.GeneratedQuery );
		}
		...
		public void TestValueTypeRelation()
		{
			...
			Assert.AreEqual( String.Format("SELECT [Sozialversicherungsnummer].[Nummer] FROM [Sozialversicherungsnummer] INNER JOIN [Mitarbeiter] ON [Mitarbeiter].[ID] = [Sozialversicherungsnummer].[IDSozial] WHERE [Mitarbeiter].[Position_X] > 2 AND [Mitarbeiter].[Position_Y] < 5", this.mitarbeiterFields), q.GeneratedQuery );
		}
		...
		public void CheckIfMultiKeysWork()
		{
			...
			Assert.AreEqual( "SELECT [OrderDetail].[Price] FROM [OrderDetail] WHERE [OrderDetail].[IDProduct] = {0} AND [OrderDetail].[IDOrder] = {1}", q.GeneratedQuery );
			bool thrown = false;
			try
			{
		...
		public void TestSuperclasses()
		{
			...
			Assert.AreEqual( String.Format( "SELECT {0} FROM [Beleg];\nSELECT {1} FROM [PKWFahrt];", this.belegFields, this.pkwFahrtFields ), qk.GeneratedQuery );
		}
		...
		public void TestPolymorphicRelationQueries()
		{
			...
			Assert.AreEqual( "SELECT [Reise].[Zweck] FROM [Reise] INNER JOIN [relBelegKostenpunkt] ON [Reise].[ID] = [relBelegKostenpunkt].[IDReise] INNER JOIN [Beleg] ON [Beleg].[ID] = [relBelegKostenpunkt].[IDBeleg] AND [relBelegKostenpunkt].[TCBeleg] = 926149172 WHERE [Beleg].[Datum] = {0} UNION \r\nSELECT [Reise].[Zweck] FROM [Reise] INNER JOIN [relBelegKostenpunkt] ON [Reise].[ID] = [relBelegKostenpunkt].[IDReise] INNER JOIN [PKWFahrt] ON [PKWFahrt].[ID] = [relBelegKostenpunkt].[IDBeleg] AND [relBelegKostenpunkt].[TCBeleg] = 734406058 WHERE [PKWFahrt].[Datum] = {0}", q.GeneratedQuery );
		}
		...
		public void Test1To1Bidirectional()
		{
			...
			Assert.AreEqual( "SELECT [Sozialversicherungsnummer].[Nummer] FROM [Sozialversicherungsnummer] INNER JOIN [Mitarbeiter] ON [Mitarbeiter].[ID] = [Sozialversicherungsnummer].[IDSozial] WHERE [Mitarbeiter].[Vorname] = 'Mirko'", qs.GeneratedQuery );
		}
		...
		public void Test1To1BiWithTable()
		{
			...
			Assert.AreEqual( "SELECT [Zertifikat].[Schlüssel] FROM [Zertifikat] INNER JOIN [relSignaturZertifikat] ON [Zertifikat].[ID] = [relSignaturZertifikat].[IDZertifikat] INNER JOIN [Signatur] ON [Signatur].[ID] = [relSignaturZertifikat].[IDSignatur] WHERE [Signatur].[Signature] = 'abc'", qz.GeneratedQuery );
			NDOQuery<Signatur> qs = new NDOQuery<Signatur>(pm, "owner.schlüssel = -4");
			string s = qs.GeneratedQuery;
			Assert.AreEqual( "SELECT [Signatur].[Signature] FROM [Signatur] INNER JOIN [relSignaturZertifikat] ON [Signatur].[ID] = [relSignaturZertifikat].[IDSignatur] INNER JOIN [Zertifikat] ON [Zertifikat].[ID] = [relSignaturZertifikat].[IDZertifikat] WHERE [Zertifikat].[Schlüssel] = -4", qs.GeneratedQuery );
		}
